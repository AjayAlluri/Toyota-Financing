...
router.post('/register', async (req, res) => {
  try {
    ...
    if (role && !['user', 'sales'].includes(role)) {
      return res.status(400).json({ error: 'Invalid role. Must be "user" or "sales"' });
    }
    ...
    const userId = await createUser(email, passwordHash, firstName, lastName, role);
